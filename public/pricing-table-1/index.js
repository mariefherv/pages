...
(function () {
  ...
  var kMp5sM = _createClass(function kMp5sM(data) {
    var _this = this;
    _classCallCheck(this, kMp5sM);
    _defineProperty(this, "render", function () {
      var currencyList = ['&#36;', '&#8364;', '&#x20B1;', '&#xa3;', '&#xFFE5;', '&#20803;'];
      var currency = currencyList[_this.data.currency.value];
      var rates = ['/hr', '/day', '/mo', '/yr'];
      document.querySelector('#content').insertAdjacentHTML('beforeend', "\n        <section id=\"".concat(attr(_this.data.id), "\" class=\"kMp5sM ").concat(_this.data.c.classes ? attr(_this.data.c.classes) : '', "\" style=\"").concat(attr(_this.data.c.section), "\">\n            <div class=\"container\" style=\"").concat(attr(_this.data.c.container), "\">\n              ").concat(_this.data.header.value, "\n              <div class=\"row\" style=\"--theme-color: ").concat(_this.data.textColor.value, "; --button-color: ").concat(_this.data.buttonColor.value, "; --button-text-color: ").concat(_this.data.buttonTextColor.value, "\">\n              ").concat(_this.data.items.map(function (item) {
        var features = item.features.value.split(', ');
        return "\n                    <div class=\"col\">\n                      <div class=\"pricing-box ".concat(item.highlight.value == 1 ? 'highlight' : '', "\">\n                        <div class=\"img-area\">\n                          <img src=").concat(item.icon.value, " alt=").concat(item.heading.value + ' image', ">\n                        </div>\n                        <div class=\"text-area\">\n                          <h3>").concat(__html(item.heading.value), "</h3>\n                          <p>").concat(__html(item.text.value), "</p>\n                          ").concat(_this.data.currency.value == 5 ? "<h1>".concat(__html(item.price.value), "<sup>").concat(currency, "</sup><sub>").concat(__html(rates[item.rate.value]), "</sub></h1>") : "<h1><sup>".concat(currency, "</sup>").concat(__html(item.price.value), "<sub>").concat(__html(rates[item.rate.value]), "</sub></h1>"), "\n                          <ul>\n                            ").concat(features.map(function (feature) {
          return "\n                                <li>".concat(__html(feature), "</li>\n                              ");
        }).join(''), "\n                          </ul>\n                          <a href=").concat(__html(item.buttonLink.value), ">").concat(__html(item.buttonText.value), "</a>\n                        </div>\n                      </div>\n                    </div>\n                  ");
      }).join(''), "\n              </div>\n          </div>\n        </section>\n        "));
    });
    this.data = data;
    this.render();
